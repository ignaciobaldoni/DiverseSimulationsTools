...
#ä Selection of outputs and saved files 
transmission_check  = False     # Transmission check for different waist scales
print_texts         = False     # Print in python console the input parameters 
plotting            = True     # Plots spectrum for every iteration (Useful when only few iterations are implemented. Otherwise, increases calculation time)
opt_Plot            = False      # Color plot of broadening achieved for all scanned parameters
opt_Analysis        = False     # Analysis is on/off for different parameters. (Useful to be off when only one single configuration is of interest. )
saveTxt             = True     # Saves output in txt file. Also if ON, saves the plot from "plotting". 
savePlotForAnalysis = False     # Saves output in txt file. Also if ON, saves the plot from "plotting". 
...
# sweeping of fiber parameters 
opt             = []  # For appending the results obtained of each iteration
Taper_Length    = [0.14]#np.linspace(0.05, 0.14,2)     # In meters
Waist_Scale     = [0.4]#np.linspace(0.3, 0.50,2)      # Adimensional. It is a ratio  
Waist_len       = 0.025                         # In meters
Waist_Length    = [Waist_len]  #np.linspace(0.010, 0.0433,2)

 
# Pynlo forces the code to be at least partly in 
# other units (nm, THz, ps), so we go along with it.

# pulse parameters 
shape       = 'sech2'       # pulse shape ('Gauss' or 'sech2')
FWHM_ps     = 0.250          # pulse duration [ps]
carrier_nm  = 1542          # pulse central wavelength [nm]
frep_MHz    = 100#12126         # pulse repetition rate [MHz]
pow_mW      = 0.6*90#7000*0.6      # average power [mW]  0.6 --> coupling efficiency at input facet
...
window_ps   = 50.0         # simulation window [ps] 50.0
steps       = 200           # simulation steps
points      = 40000         # simulation points 40000
low_nm      = 600           # start wavelength [nm]
high_nm     = 2300          # stop wavelength [nm]
saveTXT     = saveTxt       # save output spectrum as txt?
savePlot    = saveTXT       # save the plot (will go full screen)
axis        = "slow"        # polarization ("slow" or "fast")
workingDir  = "Spirou_Fiber_v2" 
# Fiber type 1 = "NL_20110305_12 (Spirou)" # path to working directory
...
# Scan of waist scale and taper length. 
# Waist length is kept fixed although can also be included in the analysis.
...
for taper_length in Taper_Length: 
    ...
    #for Waist_len in Waist_Length: 
        ...
        taper_len   = taper_length      # (down-)taper transition length [m]
        waist_len   = Waist_len         # waist length [m]
        uptap_len   = 0.02              # uptaper length [m]
        untap_scale = 1.00              # scale of fiber diam. rel. to original value
        waist_scale = Waist_scale       # scale of fiber diameter at waist
        
        propa_len   = taper_length + 2*untap_len + waist_len + uptap_len   # propagation length [m]  
        # print(taper_len,waist_scale,waist_len)
        ...
        #### Magic numbers    
        # These two magic numbers are used to avoid artefacts from the 
        # simulation process which can estimtate a gap inside the initial pulse
        # spectrum which does not exist when measured. 
        magic_number1 = 1490 
        magic_number2 = 1610
        magic_number3 = 0.45
              
        begin_iteration = np.where(np.abs(xW - magic_number1)<magic_number3)[0][0]
        end_iteration = np.where(np.abs(xW - magic_number2)<magic_number3)[0][0]        
        
        flattening_level = -30 
        # The flattening level usually stands around -30 dB or -25 dB from the 
        # maximum value of the entire spectrum (~-30 dB)
        
        # This loop searches for the index point where the spectrum reaches the 
        # flatenning level for the high wavelength region
        hh = end_iteration 
        while xW[hh]<xW[0]:
            if np.abs(dB_nm_out[hh]-(flattening_level))<0.5:
                max_wl = dB_nm_out[hh]
                break
            else:
                hh-=1
        ...
        # This loop searches for the index point where the spectrum reaches the
        # flatenning level for the low wavelength region        
        ll = begin_iteration
        while xW[ll]>xW[-1]:
            if np.abs(dB_nm_out[ll]-(flattening_level))<0.5:
                min_wl = dB_nm_out[ll]
                break
            else:
                ll+=1
        ...
        # Define the lowest and highest wavelength achieved with the supercontinuum
        # generation according to the index values found previously.            
        low_lambda = np.where(dB_nm_out == min_wl)[0][0]
        high_lambda = np.where(dB_nm_out == max_wl)[0][0]
                
        # Sprectal broadening range
 ...
 ## Once the whole script is run, we analize the result for each configuration
 ## of parameters. 
  
if opt_Analysis == True:
    
    Opt = pd.DataFrame(opt)
    new_column_names = ['Taper_length', 'Waist_scale','Waist_len',
                        'Low_wavelength','High_wavelength', 'SC_range']
    ...
    now = str(dt.datetime.now()).split(".")[0]      # split to remove ms value
    now = now.replace(":",".").replace(" ", "_")    # reformat date/time string
    Opt.to_csv('Opt'+str(pow_mW)+'mW_'+str(FWHM_ps)+'ps_'+str(now)+'.csv',index=False)

        
    
    ############################################
    ###     Plots for optimization study     ###
    ############################################    
    
    k=0
    p=0
    g=[len(Waist_Scale) if len(Waist_Scale) >= 2 else len(Waist_Length)][0]
    xAxis = [Opt.Waist_scale[k+0:k+g] if len(Waist_Scale) >= 2 else Opt.Waist_len[k+0:k+g]][0]
    Saved = ['Waist scale' if len(Waist_Scale) >= 2 else 'Waist length'][0]
    
    plt.figure()
    for h in Taper_Length:
        
        Label = 'Taper length = %s m'%(np.round(h,3))        
        plt.plot(xAxis,Opt.SC_range[k+0:k+g],'-o',label=Label,alpha=(1-p*0.1))
        k = k + g
    
    plt.legend()
    plt.grid()
    plt.title('Uptaper = %s , waist length = %s, untap_length = %s'%(uptap_len,waist_len, untap_len))
    [plt.xlabel('Waist scale') if len(Waist_Scale) >= 2 else plt.xlabel('Waist length [m]')]
    plt.ylabel('Broadening range [nm]')
    if savePlotForAnalysis: plt.savefig('Taper length'+str(Saved)+'optimization'+ now + ".png", dpi = 300)    
    ...
    for h in Taper_Length:
        ...
        Label = 'Taper length = %s m'%(np.round(h,3))
        plt.plot(xAxis,Opt.High_wavelength[k+0:k+g],'-o',label=Label,alpha=(1-p*0.1))
        k = k + g
    ...
    if savePlotForAnalysis: plt.savefig('Taper length'+str(Saved)+'opt_Higher wavelength accessed'+ now + ".png", dpi = 300)
    ...
    for h in Taper_Length:
        ...
        Label = 'Taper length = %s m'%(np.round(h,3))
        plt.plot(xAxis,Opt.Low_wavelength[k+0:k+g],'-o',label=Label,alpha=(1-p*0.1))
        k = k + g
    ...
    if savePlotForAnalysis: plt.savefig('Taper length'+str(Saved)+'opt_Lower wavelength accessed'+ now + ".png", dpi = 300)
    ...
    if opt_Plot == True:          
        ...
        x = Taper_Length
        y = Waist_Scale
        z = np.array(Opt.SC_range)
        
        Power_   = np.array(np.round(Opt.Low_wavelength,0))
        ...
        for (j,i),label in np.ndenumerate(Power):
            plt.text(i,j,str(label)+'',ha='center',va='center',fontsize=10)    
            
        positions = range(0,len(y))
        labels = np.round(y,3)
        plt.xticks(positions, labels)
    
        positions = range(0,len(x))
        labels = np.round(x,3)
        plt.yticks(positions, labels)
        plt.xlabel('Waist scale')
        plt.ylabel('PCF Tapering length [m]')
        plt.title('Supercontinuum broadening')    
            
        cbar = plt.colorbar()        
        if savePlotForAnalysis: plt.savefig('Taper length'+str(Saved)+'opt_color_Plot_'+ now + ".png", dpi = 300)
        plt.show()

    
    
    ## ------------------------------------------------------------------------ ##
    # The PCF studied here have a cutoff which defines the highest wavelength 
    # the core can transmit and depends on the waist scale value.  
    # The transmission values comes from the FEM simulations of n_eff. 
    ## ------------------------------------------------------------------------ ##
    if transmission_check:
        Scales = ['0.20','0.25','0.30','0.35','0.40']
        
        magic_number5 = 0.85105075
        
        for i in Scales:
            input_file = workingDir+"\dispersion\scale_"+str(i)+".txt"
            plot_param = "transmission (25 mm) [dB]"
            label = "LP$_{01}$ slow"
            ...
            if "transmission" in plot_param:
                ...
                if "(" in plot_param:
                    ...
                    plt.plot(wl_nm, t, label="Waist scale %s"%i)
                plt.legend()
                plt.grid()
                
            t=np.array(t,dtype=float)
            plt.plot(wl_nm[np.where(np.abs(t-(-5))<magic_number5)[0][0]],
                     t[np.where(np.abs(t-(-5))<magic_number5)][0],
                     'ro',label = 'Cutoff = %s nm '%np.round(wl_nm[np.where(np.abs(t-(-5))<magic_number5)[0][0]],2))
            if savePlotForAnalysis: plt.savefig('TransChecked_'+str(workingDir)+'_'+ now + ".png", dpi = 300)
        
            

if savePlotForAnalysis == True:
