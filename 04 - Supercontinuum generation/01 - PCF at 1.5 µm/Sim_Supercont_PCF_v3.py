...
# Transmission checking for different scales 
...
save_plot_for_measurement_analysis = False

opt = []
Taper_Length = [0.14]#np.linspace(0.05, 0.14,15) # [0.030,0.140]
Waist_Scale = [0.4]#np.linspace(0.3, 0.50,10) #[0.3,0.20]
Waist_Length = [0.027]#np.linspace(0.010, 0.0433,2) #[0.3,0.20]
Waist_len = 0.025 #[m]

# [0.14, 0.4, 0.022, 891.92639098571, 2199.2150542740605, 1307.2886632883506]
# [0.14, 0.4, 0.022, 1077.5454013835667, 2199.2150542740605, 1121.6696528904938]

# [0.14, 0.4, 0.025, 1097.1043033513245, 2199.860563036997, 1102.7562596856726]
# [0.08, 0.36, 0.025, 1118.4702507967675, 2025.652959756187, 907.1827089594196]

Waist_scale = Waist_Scale[0]
for taper_length in Taper_Length: 
    ...
    #for Waist_len in Waist_Length:
        ...
        waist_scale = Waist_scale       # .3 scale of fiber diameter at waist
        
        propa_len   = taper_length + 2*untap_len + waist_len + uptap_len   # 0.20 propagation length [m]
        
        workingDir  = "Spirou_Fiber_v2"#"NL_20110305_12 (Spirou)" # path to working directory
        axis        = "slow"    # polarization ("slow" or "fast")
        
             
        # simulation parameters
        window_ps   = 50.0          #20.0 simulation window [ps]
        steps       = 200           #200 simulation steps
        points      = 40000        #24000 18400 13000 simulation points
        ...
        end_iteration = np.where(np.abs(xW - 1610)<0.45)[0][0] #1610 
        begin_iteration = np.where(np.abs(xW - 1490)<0.45)[0][0] #1490
...
if full_analysis == True:
    ...
    new_column_names = ['Taper_length', 'Waist_scale','Waist_len','Low_wavelength','High_wavelength', 'SC_range']
    ...
    plt.savefig('Taper length'+str(Saved)+'optimization'+ now + ".png", dpi = 570)
    ...
    plt.savefig('Taper length'+str(Saved)+'opt_Higher wavelength accessed'+ now + ".png", dpi = 570)
    ...
    plt.savefig('Taper length'+str(Saved)+'opt_Lower wavelength accessed'+ now + ".png", dpi = 570)
    ...
    if transmission_check:
        ...
        for i in Scales:
            ...
            # plt.plot(2420,-5,'ro',label = 'Cutoff wavelength')
            plt.plot(wl_nm[np.where(np.abs(t-(-5))<0.85105075)[0][0]],t[np.where(np.abs(t-(-5))<0.85105075)][0],'ro',label = 'Cutoff = %s nm '%
                      np.round(wl_nm[np.where(np.abs(t-(-5))<0.85105075)[0][0]],2))
    ...
    if color_plot == True: 
        ...
        plt.savefig('Taper length'+str(Saved)+'opt_color_plot_'+ now + ".png", dpi = 570)
        plt.show()

if save_plot_for_measurement_analysis == True:
    FOLDER = 'Untapered PCF'
    Folder = 'C:/Users/ibaldoni/Desktop/'+FOLDER+'/Simulation_results'
