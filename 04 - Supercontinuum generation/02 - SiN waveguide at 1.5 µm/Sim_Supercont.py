...
FWHM_ps     = .09         # pulse duration [ps]
carrier_nm  = 1560          # pulse central wavelength [nm]
frep_MHz    = 10000.2126        # pulse repetition rate [MHz]
pow_mW      = 700          # average power [mW]
GDD         = 0e-3          # linear pre-chirp [ps^2]
power_input = pow_mW*1.67   # Coupling into the waveguide ~ 60% 

# fiber parameters
nonlinear_index = 2.4e-19   # n2 for stochiometric Si3N4 [m^2/W] 
                            # 2.74e-20 = n2 for fused silica [m^2/W]
propa_len   = 0.020         # propagation length [m]
untap_len   = 0.00          # initial untapered length [m]
taper_len   = 0.00          # (down-)taper transition length [m]
waist_len   = 0.00          # waist length [m]
uptap_len   = taper_len     # uptaper length [m]
untap_scale = 1.0           # scale of fiber diam. rel. to original value
'For straight waveguides, this seems to stablish the width, in nm'
waist_scale = 0.            # scale of fiber diameter at waist
workingDir  = "SiN_Waveguide" # path to working directory
height      = 950 
...
points      = 13000     # simulation points
low_nm      = 750       # start wavelength [nm]
high_nm     = 2200      # stop wavelength [nm]
...
dispFolder = workingDir + "/dispersion/dispersion_h"+str(height)+"nm"
...
#https://opg.optica.org/DirectPDFAccess/67314F52-867D-495A-8ECB62D394E371FA_326984/oe-23-20-25827.pdf?da=1&id=326984&seq=0&mobile=no
...
if savePlot:
    ...
    plt.savefig(folder + "/h" +str(height)+'nm_w'+str(untap_scale)+'um_RR_'+str(frep_MHz*1e-3)+'GHz'+str(pow_mW)+'mW.png', dpi = 570)
    print("Plot saved under " + folder + "/" + now + ".png")

print('Power before coupling:',power_input*1e-3,'W')
