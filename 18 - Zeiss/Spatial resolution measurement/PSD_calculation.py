...
if plot_combined:
    ...
    # ax[0].set_xlim([1,0.55e3])
    # ax[0].set_ylim([-150,40])
    ...
    plt.xlabel('Frequency Offset [Hz]')
    plt.ylabel(r'$\mathcal{L}$ [dBc/Hz]')
    add_grids()
    # plt.xlim([3.5e-3,0.55e3])
    plt.ylim([-150,40])
    plt.savefig('PhaseNoise.png')
...
# sqrtfflu=np.sqrt(fffluctu[0].real*Max_freq_fluct**2) # conversion to Hz²/Hz then Hz/sqrt(Hz)
...
# plt.semilogx(ffreq,phase_noise_dBc_Hz)
# plt.ylim([-150,40])
# plt.ylabel(r'$\mathcal{L}$ [dBc/Hz]')
# plt.xlabel('Frequency Offset [Hz]')
